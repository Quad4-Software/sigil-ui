...
export function attachTagsInput(
  root: HTMLElement,
  options: TagsInputOptions = {}
): TagsInputController {
  const found = root.querySelector<HTMLInputElement>('.sig-tags-field, input')
  if (!found) throw new Error('sigil: attachTagsInput needs an input inside .sig-tags-input')
  const input = found
  let tags = [...(options.tags ?? [])]

  function chip(tag: string): HTMLSpanElement {
  ...
  function render() {
    ...
    for (const tag of tags) input.before(chip(tag))
  }

  function add(raw: string) {
    const tag = raw.trim().replace(/,+$/, '')
    if (!tag) return
    if (tags.length >= (options.max ?? Infinity)) return
    if (!options.duplicates && tags.includes(tag)) return
    tags = [...tags, tag]
    render()
  ...
  const unsubs = [
    ...
    on(root, 'click', (event) => {
      const btn = (event.target as HTMLElement).closest<HTMLElement>('.sig-tag-remove')
      if (!btn || isDisabled(root)) return
      const tag = btn.closest<HTMLElement>('.sig-tag')?.dataset.value
      if (tag) remove(tag)
    })
  ]
