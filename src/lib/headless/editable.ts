...
export function attachEditable(
  root: HTMLElement,
  options: EditableOptions = {}
): EditableController {
  const foundPreview = root.querySelector<HTMLElement>('.sig-editable-preview')
  const foundInput = root.querySelector<HTMLInputElement>('.sig-editable-input')
  ...
  function apply() {
    ...
    if (!editing) preview.textContent = value || preview.textContent
  }
  ...
  const unsubs = [
    ...
    on(input, 'blur', () => {
      if (editing) (submitOnBlur ? submit : cancel)()
    })
  ]
