...
export function attachRating(root: HTMLElement, options: RatingOptions = {}): RatingController {
  const stars = [...root.querySelectorAll<HTMLElement>('.sig-rating-star, [role="radio"]')]
  if (!stars.length) throw new Error('sigil: attachRating needs .sig-rating-star buttons')
