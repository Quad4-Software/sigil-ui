...
export function attachTree(
  ...
  const nodes = () => [...root.querySelectorAll<HTMLElement>('[role="treeitem"]')]
  const idOf = (n: HTMLElement) => n.dataset.value ?? ''
  // the ul a treeitem's li sits inside
  const groupOf = (n: HTMLElement) => n.parentElement?.closest('ul') ?? null
  // the sub-group a treeitem expands, a sibling ul inside the same li
  const subOf = (n: HTMLElement) =>
    n.parentElement?.querySelector<HTMLElement>(':scope > ul') ?? null
  // the treeitem that owns the group n sits in
  const parentOf = (n: HTMLElement) =>
    groupOf(n)?.parentElement?.querySelector<HTMLElement>(':scope > [role="treeitem"]') ?? null
  const visible = () =>
    nodes().filter((n) => {
      let g = groupOf(n)
      while (g) {
  ...
  const unsubs = [
    on(root, 'click', (event) => {
      const n = (event.target as HTMLElement).closest<HTMLElement>('[role="treeitem"]')
      if (!n || isDisabled(n)) return
      const id = idOf(n)
      if ((event.target as HTMLElement).closest('.sig-tree-chevron') && subOf(n)) toggle(id)
    ...
    on(root, 'keydown', (event) => {
      ...
      if (event.key === 'ArrowDown' && at < vis.length - 1) {
        ...
        event.preventDefault()
        if (expanded.has(id) && sub) toggle(id)
        else parentOf(n)?.focus()
