...
export function attachFileUpload(root, options = {}) {
    const dropzone = root.querySelector('.sig-dropzone');
    const input = root.querySelector('input[type="file"]');
    const list = root.querySelector('.sig-file-list');
    if (!dropzone || !input)
    ...
    const unsubs = [
        ...
        on(root, 'click', (event) => {
            const rm = event.target.closest('.sig-file-remove');
            if (!rm)
                return;
            const item = rm.closest('.sig-file-item');
