...
export function createPopover(options) {
    ...
    function place() {
        ...
        let y = 0;
        if (side === 'bottom')
            y = rect.bottom + sy + offset;
        else if (side === 'top')
            y = rect.top + sy - box.height - offset;
        else if (side === 'right')
            x = rect.right + sx + offset;
        else
            x = rect.left + sx - box.width - offset;
        if (side === 'bottom' || side === 'top') {
            if (align === 'start')
                x = rect.left + sx;
            else if (align === 'end')
                x = rect.right + sx - box.width;
            else
                x = rect.left + sx + (rect.width - box.width) / 2;
        }
        else {
            if (align === 'start')
                y = rect.top + sy;
            else if (align === 'end')
                y = rect.bottom + sy - box.height;
            else
                y = rect.top + sy + (rect.height - box.height) / 2;
        }
    ...
    function show() {
        ...
        unsubs = [
            on(document, 'pointerdown', (event) => {
                ...
                if (!content.contains(t) && !trigger.contains(t))
                    close();
            }, { capture: true }),
