...
export function attachPaneGroup(root) {
    const direction = root.dataset.direction === 'vertical' ? 'vertical' : 'horizontal';
    const panes = [...root.querySelectorAll(':scope > .sig-pane')].map((el) => ({
        el,
        min: Number(el.dataset.min ?? 0),
        max: Number(el.dataset.max ?? 100)
    }));
    const resizers = [...root.querySelectorAll(':scope > .sig-pane-resizer')];
    const saveKey = root.dataset.autosave;
    let sizes = [];
    try {
        const raw = saveKey ? localStorage.getItem(`sig-panes-${saveKey}`) : null;
        if (raw)
            sizes = JSON.parse(raw).slice(0, panes.length);
    }
    catch {
        // malformed saved layout
    }
    if (sizes.length !== panes.length) {
        const declared = panes.map((p) => p.el.dataset.defaultSize === undefined ? null : Number(p.el.dataset.defaultSize));
        const used = declared.reduce((sum, s) => sum + (s ?? 0), 0);
    ...
    resizers.forEach((resizer, i) => {
        ...
        let startSizes = [];
        let startPos = 0;
        unsubs.push(on(resizer, 'pointerdown', (event) => {
            ...
            if (!('dragging' in resizer.dataset))
                return;
            ...
            if (event.key === 'Home')
                delta = -(sizes[i] ?? 0) + (panes[i]?.min ?? 0);
            else if (event.key === 'End')
                delta =
                    Math.min(panes[i]?.max ?? 100, (sizes[i] ?? 0) + (sizes[i + 1] ?? 0) - (panes[i + 1]?.min ?? 0)) - (sizes[i] ?? 0);
            if (delta === 0)
                return;
            event.preventDefault();
