...
export function attachTabs(root, options = {}) {
    ...
    tabs.forEach((tab, i) => {
        ...
        if (!tab.id)
            tab.id = nextId('sig-tab');
        const panel = panels[i] ?? null;
        if (panel) {
            if (!panel.id)
                panel.id = nextId('sig-panel');
    ...
    const unsubs = [
        ...tabs.map((tab) => on(tab, 'click', () => {
            if (!isDisabled(tab) && tab.dataset.value)
                select(tab.dataset.value);
        })),
        on(list, 'keydown', (event) => {
            const tab = tabs.find((t) => t === event.target || t.contains(event.target));
            if (!tab)
                return;
